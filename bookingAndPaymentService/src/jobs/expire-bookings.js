...
cron.schedule("*/5 * * * *", async () => {
  try {
    ...
    logger.info(`Found ${expiredBookings.length} expired booking(s). Processing...`);
    ...
    for (const booking of expiredBookings) {
      try {
        ...
        logger.info(`Expired booking #${booking.id}, released ${booking.noOfSeats} seat(s)`);
      } catch (error) {
        logger.error(`Failed to expire booking #${booking.id}: ${error.message}`);
      }
    }
  } catch (error) {
